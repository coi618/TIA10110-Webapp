# TIA101G2-product DB 
CREATE DATABASE IF NOT EXISTS G2_product; # 建立
-- DROP DATABASE G2_product; # 刪除
USE G2_product; # 指定使用DB
# --------- 刪除表格: product_detail > product_img > (> product_up_img) > product_order > product ---------
DROP TABLE IF EXISTS product_detail;
DROP TABLE IF EXISTS product_img;
DROP TABLE IF EXISTS product_up_img;
DROP TABLE IF EXISTS product_order;
DROP TABLE IF EXISTS product;


# --------- 建立 table: product_order > product > product_detail > product_img (> product_up_img)---------
# product_order
CREATE TABLE product_order ( # CREATE product_order TABLE
	...
	ord_status int comment '訂單狀態 - 0:未完成, 1:已完成', # 新增: 2:取消
...
# Addition practice ---------------- 
# product_up_img
CREATE TABLE product_up_img ( # CREATE my_detail TABLE
	prod_up_img_id int AUTO_INCREMENT PRIMARY KEY comment '圖片ID',
	prod_id int comment '商品ID', #  NOT NULL
	img_src longblob comment '圖片檔', # 上傳練習
	img_name varchar(255) comment '圖片名稱', 
	FOREIGN KEY (prod_id) REFERENCES product(prod_id) 
);
# Increment with default value XXX
ALTER TABLE product_up_img AUTO_INCREMENT=15000001;

# --------- 加入資料: product > product_img (> product_up_img) > product_order > product_detail ---------
# product
...
-- DROP TABLE IF EXISTS product_img;
-- 移除DB (沒人敢用的指令，很危險)
#DROP DATABASE G2_product;

-- Test by mysql commands ------
# Delete not exists product
-- DELETE FROM product WHERE prod_id = 2; # Updated ROWS: 0
# Query not exists product
-- SELECT * FROM product WHERE prod_id = 2; # N/A, Nothing SHOW up 
-- SELECT * FROM product WHERE prod_id = 13000001;
